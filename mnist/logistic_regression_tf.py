""" Like logistic_regression_np.py but constructs the computation graph for
the model, the cost, gradient of the cost  and the accuracy in tensorflow. 
"""
...
def main():
    ...
    W = tf.placeholder(tf.float32, [dim_phi, K])
    ...
    dcost = dcost_fun(y, t, phi, batch_size)

    accuracy = accuracy_fun(y, t)

    # numpy weight matrix for feeding into tf computation graph
    nW = np.zeros((dim_phi, K))
    ...
    for epoch in xrange(max_epochs):
        ...
        for end_index in xrange(batch_size, num_examples, batch_size):
            ...
            # run the graph to compute cost and dcost
            [ncost, ndcost] = sess.run([cost, dcost],  feed_dict={phi: phi_batch, t: t_batch, W: nW})
            
            # weight update
            nW = nW - eta * ndcost

        # run the graph to compute the accuracy for the test set
        naccuracy = sess.run(accuracy,  feed_dict={phi: test_phi, t: test_t, W: nW})
