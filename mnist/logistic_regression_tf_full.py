""" Like logistic_regression_tf_gradient.py but perform the weight update in the 
computation graph.
"""
...
def main():
    ...
    # node for performing the weight update in the computation graph
    update_W = tf.assign(W, W - eta * dcost[0])
    ...
    for epoch in xrange(max_epochs):
        ...
        for end_index in xrange(batch_size, num_examples, batch_size):
            ...
            # run the graph to compute the cost and the weight update
            [ncost, nupdate_W] = sess.run([cost, update_W],  feed_dict={phi: phi_batch, t: t_batch})
